...
/**
 * @brief 赛道线识别（忠实移植赛曙官方例程 tracking.cpp）
 *        提取赛道左右边缘、色块宽度、岔路信息、车库/斑马线识别
 */
...
public:
    ...
    vector<POINT> inlines;              // 记录赛道内的异常点（岔路）
        ...
        for (int row = rowStart; row > rowCutUp; row--) // 提取 row 范围
        {
            ...
            else // 其它行色块坐标处理
            {
                ...
                if (mblock.size() == 0) { // 如果没有发现联通色块，则图像搜索完成，结束任务
                    ...
                    cornerEnable = false;
                    spurroadEnable = false;
                } else if (mblock.size() > 1) { // 存在多个色块，择优处理：选取与上一行最近的色块
    ...
    void FindBlockinRow(int row) {
        ...
        for (int col = 1; col < COLSIMAGE; col++) {
            if (!flag) {
                if (imagePath.at<uchar>(row, col) > 127 && imagePath.at<uchar>(row, col - 1) <= 127) {
                    start = col;
                }
                else if (imagePath.at<uchar>(row, col) <= 127 && imagePath.at<uchar>(row, col - 1) > 127) {
        ...
        if (imagePath.at<uchar>(row, COLSIMAGE - 1) > 127 && COLSIMAGE - 1 - start > white_limit) {
            POINT tmp(start, COLSIMAGE - 1);
            block.push_back(tmp);
        }
        else if (imagePath.at<uchar>(row, COLSIMAGE - 1) < 127 && tmp_end - start > white_limit) {
            POINT tmp(start, tmp_end);
            block.push_back(tmp);
        }
    }
        ...
        if (right != 0) {
            ...
            for (int i = 0; i < pointsEdgeLeft.size(); i++) {
                if (pointsEdgeLeft[i].y < 2) {
                    ...
                    if (counter > threshold) return index;
                    counter = 0; // 官方原样为 counter==0（笔误），此处修正为赋值
                }
            }
        }
        return index; // 官方原样为 if(threshold>10) return index（恒真），等价于 return index
    }
